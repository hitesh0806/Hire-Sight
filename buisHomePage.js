//function loadPage(){
    ...
    const loggedInData = JSON.parse(localStorage.getItem('loggedInDataBuis'));
        ...
        matchingCandidateArray.forEach((element) => {
            if(id == element.personalData.Contact){
                ...
                <div>
                    ...
                    <button class = 'js-connectBtn' >Connect</button><br><br>
                    <button>Back</button><br><br>
                </div>
                `;
                loadConnect(element.personalData.Contact);
            }
        })
    }

    function loadConnect(id){
    document.querySelector('.js-connectBtn').addEventListener('click', () => {
        console.log("click", typeof(id), id);
        let message = {
            html : `
        <div>
            <div>
                <h4>Would you like to connect with us??</h4>
                <button class = 'js-decision' data-decision = 'yes' >Yes</button>
                <button class = 'js-decision' data-decision = 'no' >No</button>
            <div>
        <div>
        `,
            company : loggedInData
        };
